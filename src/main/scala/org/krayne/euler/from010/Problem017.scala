...
object Problem017 extends Problem[Long] {
  val value: Long = {
    ...
    val tensCounts = Vector(0, -1, 6, 6, 5, 5, 5, 7, 7, 6)
    ...
    def wordify(n: Int): Int = {
      def hundreds = hundredsCounts(n/100.toInt) + (if(n >= 100) "hundred".length else 0) + (if(n > 100 && n % 100 != 0) "and".length else 0)
      def tens = specialTensCounts.get(n % 100).getOrElse(tensCounts((n % 100)/10))
      def ones = onesCounts(n % 10)
      hundreds + tens + ones
    }
    
    "onethousand".length + (1 to 999).map(wordify).sum
  }
}